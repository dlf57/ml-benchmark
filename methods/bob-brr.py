...
# store regressor
joblib_file = "/zfs1/ghutchison/geoffh/ANI/BOB-BRR-joblib.pkl"
...
for sdf in sorted(glob.iglob('/ihome/ghutchison/dlf57/ml-benchmark/molecules/stretch/*/sdf/*.sdf'), key=numericalSort):
    ...
    if name != 'HF':
        ...
        dict1.update({'bobbrr_energy': pred})
        ...
        dict1.update({'name': name})
        dict1.update({'point': pt})
        dict1.update({'bobbrr_energy': np.nan})
        results.append(dict1)
        print('{} \t{} \t{}'.format(name, pt, pred))

df = pd.DataFrame(results, columns=['name', 'point', 'bobbrr_energy'])
df.to_csv('/ihome/ghutchison/dlf57/ml-benchmark/data/bob-brr.csv', index=False)
...
for sdf in sorted(glob.iglob('/ihome/ghutchison/dlf57/ml-benchmark/molecules/dihedral/*/sdf/*.sdf'), key=numericalSort):
    ...
    d.update({'bobbrr_energy': pred})
    results.append(d)
    print('{} \t{} \t{} \t{} \t{}'.format(name, phi, psi, theta, pred))
    
df = pd.DataFrame(results, columns=['name', 'phi', 'psi', 'theta', 'bobbrr_energy'])
df.to_csv('/ihome/ghutchison/dlf57/ml-benchmark/data/bob-brr-dihedral.csv', index=False)